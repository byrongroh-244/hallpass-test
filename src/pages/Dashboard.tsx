...
export default function Dashboard() {
  ...
  const lsKey = useCallback((base: string) => `${base}_${teacherId}`, [teacherId])
  ...
  const [day, setDay] = useState<ScheduleDay>(() => (localStorage.getItem(`db_day_${teacherId}`) as ScheduleDay) ?? 'red')
  const [start, setStart] = useState<StartType>(() => (localStorage.getItem(`db_start_${teacherId}`) as StartType) ?? 'regular')
  const [periodName, setPeriodName] = useState(() => localStorage.getItem(`db_period_${teacherId}`) ?? '')

  useEffect(() => {
    localStorage.setItem(lsKey('db_day'), day)
    localStorage.setItem(lsKey('db_start'), start)
    localStorage.setItem(lsKey('db_period'), periodName)
      ...
      await Promise.all(
        Object.entries(all)
          ...
          .map(([key, s]) => writeStaleReset({
            teacherId, name: s.name, period: s.period, schedule: s.schedule,
  ...
  }, [teacherId, period, isPeriodActive, sched])
      ...
      <div style={{ padding: '1rem 1.5rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1rem', gap: 12 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
            ...
            />
            <NavIcon to={`/t/${teacherId}/analytics`} title="Analytics">
            ...
            <NavIcon to={`/t/${teacherId}`} title="Home">
