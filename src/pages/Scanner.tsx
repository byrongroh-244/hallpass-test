...
/** Namespaces every localStorage key by teacherId — same single-slot
 * mechanism as before the multi-teacher change, just scoped so two
 * teachers' devices (or two teachers using the same shared iPad) never
 * stomp on each other's day/period/max-out choices. */
function lsKeyFor(teacherId: string, base: string): string {
  return `${base}_${teacherId}`
}
...
export default function Scanner() {
  ...
  const lsKey = useCallback((base: string) => lsKeyFor(teacherId, base), [teacherId])

  const [day, setDay] = useState<ScheduleDay>(() =>
    (localStorage.getItem(lsKeyFor(teacherId, 'hp_day')) as ScheduleDay) ?? 'red')
  const [start, setStart] = useState<StartType>(() =>
    (localStorage.getItem(lsKeyFor(teacherId, 'hp_start')) as StartType) ?? 'regular')
  const [periodName, setPeriodName] = useState<string>(() =>
    localStorage.getItem(lsKeyFor(teacherId, 'hp_period')) ?? SCHEDULES.red.regular[0].name)
  const [screen, setScreen] = useState<'main'|'settings'|'firstRun'|'pickStart'|'swipe'|'welcome'>(() =>
    localStorage.getItem(lsKeyFor(teacherId, 'hp_day')) ? 'main' : 'firstRun')
  ...
  const [pickMaxOut, setPickMaxOut] = useState<number>(() => parseInt(localStorage.getItem(lsKeyFor(teacherId, 'hp_maxOut')) ?? '5'))
  ...
  const [maxOut, setMaxOut] = useState<number>(() => {
    const saved = localStorage.getItem(lsKeyFor(teacherId, 'hp_maxOut'))
    return saved ? parseInt(saved) : 5
  })
  const [maxTripMinutes, setMaxTripMinutes] = useState<number>(() => {
    const saved = localStorage.getItem(lsKeyFor(teacherId, 'hp_maxTripMinutes'))
    return saved ? parseInt(saved) : 40
  })
  const [pickMaxTripMinutes, setPickMaxTripMinutes] = useState<number>(() =>
    parseInt(localStorage.getItem(lsKeyFor(teacherId, 'hp_maxTripMinutes')) ?? '40'))
  const [adminPin, setAdminPin] = useState<string>('0244')
  ...
  useEffect(() => {
    ...
    if (!valid) {
      ...
      setPeriodName(first)
      localStorage.setItem(lsKey('hp_period'), first)
    }
  }, [day, start, periodName, lsKey])
  ...
  useEffect(() => {
    ...
    if (active && active.name !== periodName) {
      ...
      localStorage.setItem(lsKey('hp_period'), active.name)
    }
  ...
  const runResetCheckInner = async () => {
    ...
    for (const [key, student] of Object.entries(all)) {
      ...
      if (outDateStr !== today) {
        await writeStaleReset({ teacherId, name: student.name, period: student.period, schedule: student.schedule, studentKey: key, outStart, resetTime, date: today })
        continue
      }
      ...
      if (periodOver || exceededMaxTrip) {
        await writeAutoReset({ teacherId, name: student.name, period: student.period, schedule: student.schedule, studentKey: key, outStart, resetTime, date: today })
      } else {
  ...
  }, [teacherId])  // Re-subscribe only if teacherId changes — snapshot data stored in ref
  ...
  const applySchedule = (d: ScheduleDay, s: StartType) => {
    setDay(d); setStart(s)
    localStorage.setItem(lsKey('hp_day'), d)
    localStorage.setItem(lsKey('hp_start'), s)
  ...
  const commitSwipe = useCallback(() => {
    ...
    if (action === 'out') {
      ...
      setSwipeProgress(0)
      setScreen('main')
      writeStudentOut({ teacherId, day: d, start: s, name, period: p.name, outTime: now, date: today })
      ...
      writeStudentIn({ teacherId, day: d, start: s, name, period: p.name, outStart, inTime: now, date: today })
        ...
        <Overlay>
          <div style={{ background: C.white, borderRadius: 16, padding: '24px', width: isIPadLandscape ? 420 : 340, maxHeight: '65vh', overflowY: 'auto' }}>
            ...
            <button onClick={() => {
              ...
              localStorage.setItem(lsKey('hp_day'), pickDay); localStorage.setItem(lsKey('hp_start'), pickStart); localStorage.setItem(lsKey('hp_period'), pickPeriod)
              setScreen('main')
              ...
              <div style={{ marginTop: 20 }}>
                ...
                <button onClick={() => {
                  if (pickMaxTripMinutes !== maxTripMinutes) {
                    ...
                    localStorage.setItem(lsKey('hp_maxTripMinutes'), String(pickMaxTripMinutes))
                    setScreen('main')
                  }
                  ...
                  onChange={e => {
                    ...
                    if (val && i === 3) {
                      ...
                      if (full === adminPin) {
                        setPinTarget(null)
                        if (pinTarget === 'home') {
                          setScreen('main'); navigate(`/t/${teacherId}`)
                        } else {
                          setMaxOut(pickMaxOut)
                          localStorage.setItem(lsKey('hp_maxOut'), String(pickMaxOut))
                          setScreen('main')
                        }
                      } else {
