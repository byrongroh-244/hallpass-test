...
type Screen = 'main' | 'period'
interface ActivePeriod { day: DayKey; period: PeriodNum }

// No PIN gate here by design — per the multi-teacher redesign, the roster
// editor is scoped to a teacher's own /t/:teacherId subtree and the one
// universal PIN now only protects the Scanner (leaving the kiosk, changing
// student limits), not the editor.
...
export default function Editor() {
  ...
  const handleFullSave = useCallback(async (data: RosterData, mode: 'sync' | 'replace') => {
    if (mode === 'replace') {
      await replaceFullRoster(teacherId, data)
    } else {
      await saveFullRoster(teacherId, data)
    }
    setSaved(true)
  ...
  const handlePeriodSave = useCallback(async (day: DayKey, period: PeriodNum, data: RosterPeriod) => {
    await savePeriod(teacherId, day, period, data)
  }, [teacherId])
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '1.5rem' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem' }}>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            ...
            <Link to={`/t/${teacherId}`} style={{ width: 34, height: 34, borderRadius: 8, background: C.cloud, border: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', textDecoration: 'none', color: C.slate }}>
